		...
		<div class="panel-body">
			<div id="content1">	
				...
				<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
					<div class="form-group">
	                    ...
	                    <select name="xmetodo" class="form-control selectpicker" id="xmetodo" data-live-search="true">
							@foreach($metodoP as $metodo)
	                        	<option value="{{$metodo->MetodoPago}}">{{$metodo->MetodoPago}}</option>
	                        @endforeach  
	                    </select>
					</div>
				</div>
			...
			<div id="content2" style="display: none;">					
				...
				<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
					<div class="form-group">
						<label>Tipo Gasto</label>
	                    <select name="gasto" class="form-control selectpicker" id="gasto" data-live-search="true" onchange="javascript:showFechaGasto()">
...
<script>
	...
	function RequeridMin(){
		document.getElementById("fecha_factura").min = '{{date("Y-m-d", strtotime($folio->fecha_salida))}}';
		document.getElementById("fecha_factura").max = '{{date("Y-m-d", strtotime($folio->fecha_llegada))}}';
	}
	...
	function showFechaGasto(){
		//SI EL CONCEPTO DE LA COMPROBACION ES DEVOLUCION PERMITIR SELECCIONAR UNA FECHA DESPUES DE LA FECHA DEL FOLIO
		gasto = $('#gasto').val();
		// alert(gasto);
		if(gasto == 8){
			//alert('Aqui se hara el tema del calendario');
			document.getElementById("fecha_factura").min = '{{date("Y-m-d", strtotime($folio->fecha_salida))}}';
			document.getElementById("fecha_factura").max = '{{date("Y-m-d")}}';
		}
		else{
			//alert('Aqui no existe el calendario');
			// max="{{date('Y-m-d', strtotime($folio->fecha_llegada))}}"
			document.getElementById("fecha_factura").min = '{{date("Y-m-d", strtotime($folio->fecha_salida))}}';
			document.getElementById("fecha_factura").max = '{{date("Y-m-d", strtotime($folio->fecha_llegada))}}';
		}
	}
