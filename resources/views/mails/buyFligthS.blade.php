      ...
      <table align="center" border="0">
        <tr>
          <td>
            ...
            <p>Se ha autorizado el anticipo para  la solicitud de viaje con el  Número de Folio <strong>{{$folioMail->id_header_folio}}</strong>, favor de comprar los vuelos y reservar el hotel antes del día: <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong> según los siguientes datos:</br >
            <strong>{{$folioMail->criterio}}</strong></br></p>
            <p>Has been authorized the advance payment for the trip request with Folio Number <strong>{{$folioMail->id_header_folio}}</strong>, please buy the flights and book the hotel before the day: <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong> according to the following data:</br >
