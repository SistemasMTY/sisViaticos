      ...
      <table align="center" border="0">
        <tr>
          <td>
            ...
            <p><strong>{{$folioMail->NombreAuto}}</strong> ha autorizado la solicitud de viaje con el Número de Folio <strong>{{$folioMail->id_header_folio}}</strong>, por lo cual el personal <strong>{{$folioMail->name}}</strong> estará ausente los días <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong> al <strong>{{date('d-m-Y', strtotime($folioMail->fecha_llegada))}}</strong>.</p>
            <p><strong>{{$folioMail->NombreAuto}}</strong> has been authorized the travel request with Folio Number <strong>{{$folioMail->id_header_folio}}</strong>, for which the <strong>{{$folioMail->name}}</strong> staff will be absent on  <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong> at <strong>{{date('d-m-Y', strtotime($folioMail->fecha_llegada))}}</strong>.</p>
          </td>
        </tr>
      </table>
    </td></tr>
