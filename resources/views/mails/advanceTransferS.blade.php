      ...
      <table align="center" border="0">
        <tr>
          <td>
            ...
            <p>Se ha autorizado el anticipo para  la solicitud de viaje con el  Número de Folio <strong>{{$folioMail->id_header_folio}}</strong>, favor de realizar el anticipo a la cuenta: <strong>{{$folioMail->BancoCuenta}} / {{$folioMail->CLABE}}</strong> del Banco: <strong>{{$folioMail->Banco}}</strong> antes del día: <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong></br ></strong></br >
            </p>
            
            <p>Has been authorized the advance payment for the trip request with Folio Number <strong>{{$folioMail->id_header_folio}}</strong>,  please make the advance to the account: <strong>{{$folioMail->BancoCuenta}} / {{$folioMail->CLABE}}</strong> of the Bank: <strong>{{$folioMail->Banco}}</strong> before the day: <strong>{{date('d-m-Y', strtotime($folioMail->fecha_salida))}}</strong></br>
            </p>
          </td>
        </tr>
      </table>
