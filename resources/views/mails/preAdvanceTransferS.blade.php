        ...
        <tr>
          <td>
            ...
            @if($folioMail->anticipo==0)
              <p>
                <strong>{{$folioAuto->autorizador}}</strong> ha autorizado la solicitud viaje con el  Número de Folio <strong>{{$folioMail->id_header_folio}}</strong> con anticipo <strong>$ {{$folioMail->anticipo}}</strong> del usuario <strong>{{$folioMail->name}}</strong>.
              </p>
            @else
              <p>
                <strong>{{$folioAuto->autorizador}}</strong> ha autorizado la solicitud viaje con el  Número de Folio <strong>{{$folioMail->id_header_folio}}</strong>, el usuario <strong>{{$folioMail->name}}</strong> ha requerido anticipo pero cuenta con comprobaciones pendientes o un anticipo ya asignado a una fecha previa de este Folio.
              </p>
              <p>
