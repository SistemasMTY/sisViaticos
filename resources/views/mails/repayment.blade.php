      ...
      <table align="center" border="0">
        <tr>
          <td>
            ...
            <p>
              El departamento de Cuentas por Pagar ha autorizado la comprobación de gastos con el No de Folio <strong>{{$folioMail->id_header_folio}}</strong>, favor de realizar el reembolso al solicitante <strong>{{$folioMail->name}}</strong> a la siguiente cuenta/clave: <strong>{{$folioMail->BancoCuenta}} / {{$folioMail->CLABE}}</strong> del Banco: <strong>{{$folioMail->Banco}}</strong>.
            </p>
            <p>
              The Accounts Payable department has authorized the verification of expenses with the No of Folio <strong>{{$folioMail->id_header_folio}}</strong>, please make the reimbursement to the applicant <strong>{{$folioMail->name}}</strong> to the following account / code: <strong>{{$folioMail->BancoCuenta}} / {{$folioMail->CLABE}}</strong> of the Bank: <strong>{{$folioMail->Banco}}</strong>.
            </p>
          </td>
        </tr>
      </table>
            ...
            @if (count($detalles)>0)
                  ...
                  <table border="0" class="tablaGasto">
                    ...
                    @if($folioMail->tipo =="Nacional")
                      ...
                      <td width="20%"><strong>PAGADO EN EFECTIVO</strong></td>
                      <td width="20%"><strong>PAGADO EN EFECTIVO (PESOS)</strong></td>
                      @endif
                    </tr>
                    <?php 
                $amex = 0;
